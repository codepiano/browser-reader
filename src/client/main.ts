...
function importView(error = '') {
  shell(`<main class="landing"><div class="eyebrow">ONE-TIME READING WORKSPACE</div><h1>让一本书，<em>安静地进入</em><br/>你的共读。</h1><p class="lede">导入一本 EPUB 或 Markdown 书籍，整理结构后只显示当前章节。页面保持干净，适合与你正在使用的 AI 浏览器插件一起阅读。</p><div class="dropzone" id="dropzone"><div class="drop-icon">↥</div><strong>把书拖到这里</strong><span>支持无 DRM EPUB、完整 Markdown 文件或 Markdown 文件夹</span><div class="import-actions"><button class="primary" data-action="epub">选择 EPUB</button><button class="secondary" data-action="file">选择 Markdown 文件</button><button class="secondary" data-action="folder">选择 Markdown 目录</button></div><input id="epub-input" type="file" accept=".epub,application/epub+zip" hidden/><input id="file-input" type="file" accept=".md,text/markdown" hidden/><input id="folder-input" type="file" webkitdirectory multiple hidden/></div>${error ? `<div class="error">${esc(error)}</div>` : ''}<div class="privacy"><span>⌁</span> 内容只在本机临时处理，结束后可一键删除</div></main>`); setupEvents();
}
...
function setupEvents() {
  ...
  dropzone?.addEventListener('drop', (event) => { event.preventDefault(); dropzone.classList.remove('drag'); const files = event.dataTransfer?.files; const list = [...(files || [])]; const kind = list.some((file) => /\.epub$/i.test(file.name)) ? 'epub' : list.length === 1 && /\.md$/i.test(list[0].name) && !(list[0] as any).webkitRelativePath ? 'markdown-file' : 'markdown'; importFiles(files, kind); });
}
